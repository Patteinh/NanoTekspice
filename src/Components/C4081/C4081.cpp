...
C4081::C4081(const std::string &type) {
...
Tristate C4081::compute(std::size_t pin)  {
	...
	else if (pin == 4) return (gates::_and(this->compute(5), this->compute(6)));
	else if (pin == 10) return (gates::_and(this->compute(8), this->compute(9)));
	else if (pin == 11) return (gates::_and(this->compute(12), this->compute(13)));

...
C4030::C4030(const std::string &type) {
...
Tristate C4030::compute(std::size_t pin)  {
	...
	else if (pin == 4) return (gates::_xor(this->compute(5), this->compute(6)));
	else if (pin == 10) return (gates::_xor(this->compute(8), this->compute(9)));
	else if (pin == 11) return (gates::_xor(this->compute(12), this->compute(13)));

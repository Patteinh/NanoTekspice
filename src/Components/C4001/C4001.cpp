...
C4001::C4001(const std::string &type) {
...
Tristate C4001::compute(std::size_t pin)  {
	if (pin == 3) return (gates::_nor(this->compute(1), this->compute(2)));
	else if (pin == 4) return (gates::_nor(this->compute(5), this->compute(6)));
	else if (pin == 10) return (gates::_nor(this->compute(8), this->compute(9)));
	else if (pin == 11) return (gates::_nor(this->compute(12), this->compute(13)));
